...
import model from '../assets/models/keppa-tower.glb';
import { CameraControls } from ".";
import { ContactShadows, Environment, Stats } from 'drei';

function Model() {
  const gltf = useLoader(GLTFLoader, model);
  return <primitive object={gltf.scene} position={[3.25, -3, -6]} rotation={[0, -0.5, 0]} />
}
    ...
    <>
      ...
      <CameraControls />
        ...
        <Suspense fallback={<Box />}>
          <Model />
          <ContactShadows rotation-x={Math.PI / 2} position={[0, -3, 0.2]} opacity={0.7} width={100} height={100} blur={0.4} far={90} />
        </Suspense>
        <Scrolly />
      </Canvas>
    </>
