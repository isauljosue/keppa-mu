...
import model from '../assets/models/coffee.glb';
import { CameraControls } from ".";
import { ContactShadows, Environment, Stats } from 'drei';

function Model() {
  const gltf = useLoader(GLTFLoader, model);
  return <primitive object={gltf.scene} position={[4, 1, 0]} rotation={[0, 1.25, 0]} />
}
...
function Dolly() {
  ...
  useFrame(({ clock, camera }) => {
    camera.rotation.z = 1 + Math.sin(clock.getElapsedTime()) * 0.3;
    camera.position.x = 10 + Math.sin(clock.getElapsedTime()) * 0.5;    
    camera.position.y = 2.5 + Math.cos(clock.getElapsedTime()) * 1;   
    camera.position.z = 1 + Math.cos(clock.getElapsedTime()) * 0.2;
  })
  return null
}
